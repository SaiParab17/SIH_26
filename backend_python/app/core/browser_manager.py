...
class BrowserManager:
    ...
    def __init__(
        ...
        self.browser_backend = "chromium" if not self.use_camoufox else "camoufox"
        ...
        shared_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "user_data", "shared_chrome")
        if user_data_dir:
            self.user_data_dir = user_data_dir
        else:
            self.user_data_dir = shared_dir
    ...
    async def launch(self) -> Tuple[BrowserContext, Page]:
        ...
        # Inject stealth overrides to prevent navigator.webdriver detection
        ...
        # Inject Camoufox authenticated Facebook & Instagram cookies directly into Chromium!
        try:
            from app.core.cookie_importer import inject_camoufox_cookies_into_context
            await inject_camoufox_cookies_into_context(self._context)
        except Exception as cookie_err:
            logger.warning(f"Notice injecting Camoufox cookies into Chromium: {cookie_err}")
